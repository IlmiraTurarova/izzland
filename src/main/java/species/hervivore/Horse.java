...
@NoArgsConstructor
@AllArgsConstructor
@AnimalData(idealWeight = 400, amimalsInCell = 20,moveSpeed = 4,eatTillFull = 60)
public class Horse extends Herbivore {
    ...
    public void multiply()  {
        ...
        for (int i = 0; i < Dump.animalIsland[x][y].animals.size(); i++) {
                ...
                if (Dump.animalIsland[x][y].animals.get(i) != this
                        && Dump.animalIsland[x][y].animals.get(i) instanceof Horse) {
                    couple++;
                    break;
                }
            }
            if (couple == 2) {
                synchronized (Dump.animalIsland[x][y]) {
                    Dump.animalIsland[x][y].animals.add((new Horse(x,y,weight1)));
                }
            }
        }
    }
